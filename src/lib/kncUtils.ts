...
import { getAllContractAddressesWithDecimals } from "@/lib/coingecko";
...
export const resolveTokenAddressesStandalone = async (
...
🔧 Try:
  ...
  const isNativeTokenOut =
    ...
    (tokenOutSymbol.toUpperCase() === "MATIC" && platform === "polygon-pos") ||
...
export const handleGetKyberSwapQuoteBySymbolStandalone = async ({
  tokenInSymbol,
  tokenOutSymbol,
  amount,
  platform = "ethereum",
  ...
  try {
    ...
    const resolveResult = await resolveTokenAddressesStandalone(
      tokenInSymbol,
      tokenOutSymbol,
      platform,
      selectedTokenIn,
...
export const handleExecuteKyberSwapBySymbolStandalone = async ({
  tokenInSymbol,
  tokenOutSymbol,
  amount,
  platform = "ethereum",
  ...
  selectedTokenOut?: TokenOption;
  address?: string;
  handleDefiAction?: (action: string) => Promise<void>;
  logUserAction?: (data: {
    address: string;
    agent: string;
    action: string;
  ...
  try {
    ...
    console.log(`Swap transaction submitted with hash: ${executeSwapTx.hash}`);
